...
CI_REGISTRY_IMAGE			?= ${CI_REGISTRY}/https://gitlab.com/opensourcelab/openlab-site/lab-automation
...
.build_${PYTHON_VERSION}: Dockerfile lab_automation clean__py
	${_CMD_DOCKER_BUILD} \
...
test:	## run pytest on mounted-in tests (change tests, no rebuild!)
	...
	docker run \
			--platform linux/amd64 \
			--volume $(shell pwd)/bin/:/lab_automation/bin/ \
			--volume $(shell pwd):/lab_automation:rw \
			${DOCKER_IMAGE_TAG} \
			pytest --cov-report xml:/lab_automation/bin/coverage.xml tests/
