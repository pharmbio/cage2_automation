    ...
    def get_SiLA_handler(step: MoveStep, labware: list[ContainerInfo], sila_client: ArmClient,
        ...
        class TransferHandler(ObservableProtocolHandler):
            def _protocol(self, client: ArmClient, **_kwargs):
                ...
                # announce what the GetLabware below will be given, so that an arm that can do
                # so fetches the lid already while the source device prepares itself. Arms that
                # declare the announcement do not accept PrepareForInput without it.
                mover_prepare = sila_client.LabwareTransferManipulatorController.PrepareForInput(
                    ...
                    **announcement_metadata(sila_client, intermediate_actions),
                )
